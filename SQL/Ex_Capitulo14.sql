...
/*Exercício 4*/
SELECT F.Nome FROM Funcionarios F INNER JOIN Pedidos P ON F.CodFun = P.CodFun INNER JOIN Clientes C on P.Codcli = C.Codcli AND YEAR(P.DataPed)= 1996 AND MONTH(P.DataPed)= 9 

/*Exercício 5*/
SELECT P.*, C.Descr FROM Produtos P INNER JOIN Categorias C ON P.CodCategoria = C.CodCategoria AND C.Descr = 'laticínios'
...
/*Exercício 10*/
SELECT distinct C.Descr, P.Descr FROM Categorias C INNER JOIN Produtos P ON C.CodCategoria = P.CodCategoria INNER JOIN DetalhesPed DP ON P.CodProd = DP.CodProd AND DP.Qtde <= 10 INNER JOIN Pedidos Pd ON DP.NumPed = Pd.NumPed AND YEAR(Pd.DataPed)= 1998 ORDER BY C.Descr DESC
