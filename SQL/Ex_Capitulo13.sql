...
/*Exercício 2*/
SELECT SUM(preco) AS Soma, AVG(preco) AS Media, MIN(preco) AS Menor_Preco, MAX(preco) AS Maior_Preco FROM Produtos

/*Exercício 3*/
SELECT C.Pais, COUNT(P.NumPed) AS QtdePedidos FROM Clientes C, Pedidos P WHERE C.CodCli = P.CodCli GROUP BY C.Pais ORDER BY COUNT(P.NumPed) DESC

/*Exercício 4*/
SELECT Nome, Sobrenome, Cargo, Salario, (Salario * 1.1) AS Salario_Novo FROM Funcionarios

/*Exercício 5*/
SELECT SUM(DetalhesPed.Preco) FROM Detalhesped, Pedidos WHERE (DetalhesPed.NumPed = Pedidos.NumPed) and (YEAR(Pedidos.DataEntrega)= 1997) and (MONTH(Pedidos.DataEntrega)= 5)

/*Exercício 6*/
SELECT Clientes.CodCli, Clientes.Nome, Clientes.Pais FROM Clientes, Pedidos WHERE (Pedidos.CodCli = Clientes.CodCli) and (YEAR(Pedidos.DataPed)=1997) and (MONTH(Pedidos.DataEntrega)=9)  ORDER BY (Clientes.Pais)

/*Exercício 7*/
SELECT Funcionarios.Nome, Pedidos.* FROM Funcionarios, Pedidos WHERE (Funcionarios.CodFun = Pedidos.CodFun) and Nome like 'A%'

/*Exercício 8*/
SELECT Produtos.Descr, Produtos.Unidades FROM Produtos, Fornecedores WHERE (Produtos.CodFor = Fornecedores.CodFor) and (Fornecedores.Empresa = 'Exotic Liquids')

/*Exercício 9*/
SELECT distinct Produtos.Descr FROM Produtos, DetalhesPed,Pedidos WHERE (DetalhesPed.NumPed = Pedidos.NumPed) and (Produtos.CodProd = DetalhesPed.CodProd) and (Produtos.Unidades >=50) and (YEAR(Pedidos.DataPed)=1997)

/*Exercício 10*/
SELECT distinct Categorias.Descr, Produtos.Descr FROM Produtos, Categorias, Pedidos, DetalhesPed WHERE (YEAR(Pedidos.DataEntrega) = 1997) and (DetalhesPed.Qtde >= 50) and (Categorias.CodCategoria = Produtos.CodCategoria) and (Produtos.CodProd = DetalhesPed.CodProd) and (DetalhesPed.NumPed = Pedidos.NumPed) ORDER BY Categorias.Descr DESC
